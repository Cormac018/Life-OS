...
(function(global){
  ...
  function normalize(command){
    ...
    fields(c,['contract','operation','operationId','purchaseId','versionId','expected','source','entity','resolution','reason'],'');
    check(c.contract===CONTRACT,'/contract','This purchase contract is not supported.','unsupported');check(['record','correct','refund'].includes(c.operation),'/operation','This purchase operation is not supported.','unsupported');if(c.operation==='correct')text(c.reason,'/reason',1000);else check(!own(c,'reason'),'/reason','Only a correction states its reason here.');
    for(const k of ['operationId','purchaseId','versionId'])id(c[k],'/'+k,k==='purchaseId'?160:80);
    fields(c.expected,['workspaceRevision','purchaseVersionId'],'/expected');integer(c.expected.workspaceRevision,'/expected/workspaceRevision');if(c.operation==='record')check(c.expected.purchaseVersionId===null,'/expected/purchaseVersionId','A new purchase must not replace an earlier version.');else id(c.expected.purchaseVersionId,'/expected/purchaseVersionId',80);
    ...
    if(c.operation==='refund'){normalizeRefund(c);normalizeResolution(c);return c;}
    ...
    normalizeResolution(c);
    return c;
  }
  function normalizeResolution(c){
  ...
  function normalizeRefund(c){
    fields(c.entity,['date','reason','accountId','expectedPaymentVersionId','lines'],'/entity');const e=c.entity;
    check(date(e.date),'/entity/date','Choose a valid refund or return date.');text(e.reason,'/entity/reason',1000);id(e.accountId,'/entity/accountId');id(e.expectedPaymentVersionId,'/entity/expectedPaymentVersionId');
    list(e.lines,'/entity/lines');check(e.lines.length>0,'/entity/lines','Add at least one refunded or returned line.');unique(e.lines,'lineId','/entity/lines');
    for(const [i,l] of e.lines.entries()){const path='/entity/lines/'+i;fields(l,['lineId','amountPence','returnedMilligrams','order','stockObservationId'],path);id(l.lineId,path+'/lineId',80);integer(l.amountPence,path+'/amountPence');integer(l.returnedMilligrams,path+'/returnedMilligrams');check(l.amountPence>0||l.returnedMilligrams>0,path,'State money refunded, food actually returned, or both.');if(l.order!==undefined)check(l.order==='after-count',path+'/order','Before-count returns need a later ordering capability.','unsupported');if(l.stockObservationId!==undefined)id(l.stockObservationId,path+'/stockObservationId');if(!l.returnedMilligrams)check(l.order===undefined&&l.stockObservationId===undefined,path,'A money-only refund carries no stock ordering evidence.');}
    sum(e.lines.map(l=>l.amountPence),'/entity/lines');
  }
  function effectiveVersions(domain){const rows=new Map();for(const v of domain.versions)if(v.command.operation!=='refund')rows.set(v.rootId,v);return [...rows.values()];}
  function refundsFor(domain,purchaseId){return domain.versions.filter(v=>v.rootId===purchaseId&&v.command.operation==='refund');}
  function refundRemaining(domain,purchaseId){const current=effectiveVersions(domain).find(v=>v.rootId===purchaseId);if(!current)return [];const refunds=refundsFor(domain,purchaseId);return current.command.entity.lines.map(l=>{let refunded=0,returned=0;for(const v of refunds){const row=v.command.entity.lines.find(r=>r.lineId===l.id);if(row){refunded+=row.amountPence;returned+=row.returnedMilligrams;}}return {lineId:l.id,description:l.description,refundablePence:l.netPence-refunded,returnableMilligrams:(l.inventory.mode==='receive'?l.quantity.amount:0)-returned,foodId:l.inventory.foodId||null};});}
  ...
  function receiptCategory(c){const categories=[...new Set(c.entity.lines.map(l=>l.category))];return categories.length===1?categories[0]:'Shopping';}
  function makeRecordIds(c,prior=null){if(c.operation==='refund'){const paid=sum(c.entity.lines.map(l=>l.amountPence),'/entity/lines'),moneyId=paid?'purchase-refund_'+c.versionId:null;return {purchaseVersionId:c.versionId,productVersionIds:[],foodPurchaseIds:[],movementIds:c.entity.lines.filter(l=>l.returnedMilligrams>0).map(l=>'purchase-return_'+c.versionId+'_'+l.lineId),transactionId:moneyId,transactionRootId:moneyId,settlementId:null};}if(c.operation==='correct'){const inventory=c.entity.lines.filter(l=>l.inventory.mode==='receive'),changed=c.entity.totalPence!==prior.command.entity.totalPence||receiptCategory(c)!==receiptCategory(prior.command)||c.entity.merchant.name!==prior.command.entity.merchant.name;return {purchaseVersionId:c.versionId,productVersionIds:c.entity.products.map(p=>p.versionId),foodPurchaseIds:inventory.map(l=>'purchase-price_'+c.versionId+'_'+l.id),movementIds:inventory.filter(l=>l.inventory.grams!==prior.command.entity.lines.find(p=>p.id===l.id).inventory.grams).map(l=>'purchase-stock_'+c.versionId+'_'+l.id),transactionId:changed?'purchase-money_'+c.versionId:prior.recordIds.transactionId,transactionRootId:prior.recordIds.transactionRootId,settlementId:c.entity.settlements[0].id};}const stem=c.versionId;const inventory=c.entity.lines.filter(l=>l.inventory.mode==='receive'),s=c.entity.settlements[0],moneyId=s.mode==='create'?'purchase-money_'+stem:s.expectedTransactionVersionId;return {purchaseVersionId:c.versionId,productVersionIds:c.entity.products.map(p=>p.versionId),foodPurchaseIds:inventory.map(l=>'purchase-price_'+stem+'_'+l.id),movementIds:inventory.map(l=>'purchase-stock_'+stem+'_'+l.id),transactionId:moneyId,transactionRootId:s.mode==='create'?moneyId:s.transactionRootId,settlementId:s.id};}
  ...
  function prepare(command,context){
    try{
      ...
      if(c.operation!=='record')return prepareAmendment(c,context,domain,payloadDigest);
      ...
      const mark=fingerprint(c),matchingRoots=new Set(domain.versions.filter(v=>v.command.operation!=='refund'&&(fingerprint(v.command)===mark||(c.source.documentHash&&v.command.source.documentHash===c.source.documentHash))).map(v=>v.rootId)),duplicates=effectiveVersions(domain).filter(v=>matchingRoots.has(v.rootId)).map(v=>({purchaseId:v.rootId,versionId:v.id,date:v.command.entity.date,merchant:v.command.entity.merchant.name,totalPence:v.command.entity.totalPence})).concat(legacyDuplicates(c,workspace));
  ...
  function correctionRules(c,prior,domain){
    check(prior&&prior.id===c.expected.purchaseVersionId,'/expected/purchaseVersionId','This receipt changed. Review its latest version.','needs-review');
    check(!refundsFor(domain,c.purchaseId).length,'/operation','Correcting a receipt after a return or refund needs a later reconciliation capability.','unsupported');
    const old=prior.command.entity,e=c.entity;
    for(const key of ['date','time','timeZone','currency'])check(stable(e[key])===stable(old[key]),'/entity/'+key,'Purchase dates, times and currency stay fixed in this correction stage.','unsupported');
    const a=e.settlements[0],b=old.settlements[0];for(const key of ['id','date','mode','accountId','transactionRootId','expectedTransactionVersionId'])check(a[key]===b[key],'/entity/settlements/0/'+key,'The existing payment identity and dates stay fixed.','unsupported');
    check(e.lines.length===old.lines.length,'/entity/lines','Adding or removing recorded lines needs a later correction capability.','unsupported');
    for(const [i,l] of e.lines.entries()){const previous=old.lines[i],path='/entity/lines/'+i;check(l.id===previous.id&&l.kind===previous.kind,path,'Keep the existing line identities and types.','unsupported');check(l.inventory.mode===previous.inventory.mode&&l.inventory.foodId===previous.inventory.foodId,path+'/inventory','Moving a recorded line to another pantry ingredient needs a later correction capability.','unsupported');for(const key of ['order','stockObservationId'])check(l.inventory[key]===previous.inventory[key],path+'/inventory/'+key,'Keep the original stock count ordering evidence.','unsupported');}
  }
  function refundRules(c,prior,domain){
    check(prior&&prior.id===c.expected.purchaseVersionId,'/expected/purchaseVersionId','This receipt changed. Review its latest version.','needs-review');
    check(c.entity.expectedPaymentVersionId===prior.recordIds.transactionId,'/entity/expectedPaymentVersionId','Review the payment version linked to this receipt.','needs-review');
    check(c.entity.date>=prior.command.entity.date&&c.entity.date>=prior.command.entity.settlements[0].date,'/entity/date','A return or refund cannot predate its purchase or payment.');
    const remaining=refundRemaining(domain,c.purchaseId);for(const [i,l] of c.entity.lines.entries()){const r=remaining.find(r=>r.lineId===l.lineId),path='/entity/lines/'+i;check(r,path+'/lineId','This line does not belong to the receipt.');check(l.amountPence<=r.refundablePence,path+'/amountPence','This refund exceeds the amount still refundable for this line.');check(l.returnedMilligrams<=r.returnableMilligrams,path+'/returnedMilligrams','This return exceeds the food quantity still returnable for this line.');}
  }
  function currentPayment(c,prior,workspace){const payment=latestMoney(workspace.domains.money.versions).find(t=>t.rootId===prior.recordIds.transactionRootId);check(payment&&payment.id===prior.recordIds.transactionId,'/expected/purchaseVersionId','This payment was edited in Money. Reconcile that discrepancy before changing this receipt.','needs-review');check(payment.type==='expense'&&payment.amountPence===prior.command.entity.totalPence,'/expected/purchaseVersionId','The receipt payment no longer matches its recorded amount.','needs-review');return payment;}
  function amendmentMoney(c,workspace,ids,prior){
    if(c.operation==='correct'){
      if(ids.transactionId===prior.recordIds.transactionId)return null;
      const previous=workspace.domains.money.versions.find(t=>t.id===prior.recordIds.transactionId),event=makeExpense(c,workspace,ids);event.supersedes=prior.recordIds.transactionId;event.sequence=previous.sequence;if(receiptCategory(c)===receiptCategory(prior.command))event.category=previous.category;if(c.entity.merchant.name===prior.command.entity.merchant.name)event.note=previous.note;return event;
    }
    if(!ids.transactionId)return null;
    const amountPence=sum(c.entity.lines.map(l=>l.amountPence),'/entity/lines'),a=workspace.domains.money.accounts.find(a=>a.id===c.entity.accountId);
    return {id:ids.transactionId,rootId:ids.transactionRootId,supersedes:null,sequence:workspace.domains.money.sequence+1,date:c.entity.date,type:'refund',amount:amountPence/100,amountPence,accountId:a.id,toAccountId:null,debtId:null,accountName:a.name,toAccountName:'',debtName:'',category:workspace.domains.money.versions.find(t=>t.id===prior.recordIds.transactionId).category,note:(prior.command.entity.merchant.name+': '+c.entity.reason).slice(0,1000),operationId:'purchase-payment_'+c.operationId,legs:[{kind:'accounts',id:a.id,pence:amountPence}],purchaseVersionId:c.versionId,refundOfTransactionRootId:prior.recordIds.transactionRootId};
  }
  function stockBalance(movements,foodId,throughDate){
    const positions=new Map(movements.map((row,index)=>[row.id,index]));
    const ordered=movements.map((row,index)=>({row,index,anchor:row.kind==='receipt-correction'?positions.get(row.effectiveAfterMovementId):index})).filter(v=>v.row.foodId===foodId&&v.row.date<=throughDate).sort((a,b)=>compare(a.row.date,b.row.date)||a.anchor-b.anchor||a.index-b.index);
    return Math.round(ordered.reduce((n,{row})=>own(row,'observedGrams')?row.observedGrams:n+row.delta,0)*1000000)/1000000;
  }
  function prepareAmendment(c,context,domain,payloadDigest){
    const workspace=context.workspace,prior=effectiveVersions(domain).find(v=>v.rootId===c.purchaseId);
    check(!domain.versions.some(v=>v.id===c.versionId),'/versionId','This purchase version identifier already exists.');
    if(c.operation==='correct')correctionRules(c,prior,domain);else refundRules(c,prior,domain);
    check(c.entity.date<=context.today,'/entity/date','Choose a date on or before today.');currentPayment(c,prior,workspace);
    const ids=makeRecordIds(c,prior),foods=new Map(workspace.domains.food.foods.map(f=>[f.id,f])),stock=[],problems=[];
    const productMap=new Map(domain.products.map(p=>[p.versionId,p])),latestProducts=new Map();domain.products.forEach(p=>latestProducts.set(p.productId,p));
    if(c.operation==='correct'){
      for(const [i,p] of c.entity.products.entries()){const path='/entity/products/'+i,previous=latestProducts.get(p.productId);check(!productMap.has(p.versionId),path+'/versionId','Select an existing product version instead of submitting it again.');check(previous?p.supersedes===previous.versionId:p.supersedes===null,path+'/supersedes','This product changed. Review its current version.','needs-review');check(c.entity.lines.some(l=>l.productVersionId===p.versionId),path,'Every new product must be used.');if(p.ingredientMapping)check(foods.has(p.ingredientMapping.foodId),path+'/ingredientMapping','Choose a known pantry ingredient.','needs-review');productMap.set(p.versionId,p);latestProducts.set(p.productId,p);}
      for(const [i,l] of c.entity.lines.entries()){const p=productMap.get(l.productVersionId),old=prior.command.entity.lines[i];check(l.productVersionId===null||p,'/entity/lines/'+i+'/productVersionId','Choose a known product version.','needs-review');if(l.inventory.mode==='receive'){check(foods.has(l.inventory.foodId)&&p?.ingredientMapping?.foodId===l.inventory.foodId,'/entity/lines/'+i+'/inventory','The product must map to this recorded pantry ingredient.','needs-review');const grams=(l.quantity.amount-old.quantity.amount)/1000;if(grams)stock.push({lineId:l.id,foodId:l.inventory.foodId,name:foods.get(l.inventory.foodId).name,grams,previousGrams:old.inventory.grams,newGrams:l.inventory.grams,laterCount:workspace.domains.food.movements.some(m=>m.foodId===l.inventory.foodId&&own(m,'observedGrams')&&m.date>=c.entity.date)});}}
    }else for(const [i,l] of c.entity.lines.entries()){const old=prior.command.entity.lines.find(p=>p.id===l.lineId);if(l.returnedMilligrams){
      const counts=workspace.domains.food.movements.filter(m=>m.foodId===old.inventory.foodId&&m.date===c.entity.date&&['count','count-import'].includes(m.kind)),count=counts[counts.length-1],path='/entity/lines/'+i+'/returnOrder';
      if(count&&!(l.order==='after-count'&&l.stockObservationId===count.id&&acknowledged(c,path)))problems.push({path,reason:'A stock count exists on this return date. Confirm the food was returned after that count. Returning it before the count needs a later ordering capability.',status:'needs-review',stockObservationId:count.id});
      if(!count)check(l.order===undefined&&l.stockObservationId===undefined,path,'This stock-count reference no longer matches the return date.','needs-review');
      stock.push({lineId:l.lineId,foodId:old.inventory.foodId,name:foods.get(old.inventory.foodId)?.name||old.description,grams:-l.returnedMilligrams/1000,returnedMilligrams:l.returnedMilligrams,laterCount:workspace.domains.food.movements.some(m=>m.foodId===old.inventory.foodId&&own(m,'observedGrams')&&m.date>c.entity.date)});
    }}
    const accountId=c.operation==='correct'?c.entity.settlements[0].accountId:c.entity.accountId,account=workspace.domains.money.accounts.find(a=>a.id===accountId);check(account&&['current','savings'].includes(account.type),'/entity/accountId','Choose a current or savings cash account.','unsupported');check(account.openedOn<=(c.operation==='correct'?c.entity.settlements[0].date:c.entity.date),'/entity/date','This date predates the selected account.');
    const event=amendmentMoney(c,workspace,ids,prior);if(event){integer(workspace.domains.money.sequence,'/money/sequence');check(workspace.domains.money.sequence<Number.MAX_SAFE_INTEGER,'/money/sequence','The payment journal sequence is too large.');check(!workspace.domains.money.versions.some(v=>v.id===event.id||v.operationId===event.operationId),'/operationId','A generated payment identifier already exists.');ledgerCheck(workspace,event);}
    for(const [key,array] of [['foodPurchaseIds',workspace.domains.food.purchases],['movementIds',workspace.domains.food.movements]])check(!ids[key].some(id=>array.some(row=>row.id===id)),'/versionId','A generated food record identifier already exists.');
    const original=domain.versions.find(v=>v.rootId===c.purchaseId&&v.command.operation==='record'),effects=amendmentEffects(c,prior,original,ids),after=workspace.domains.food.movements.concat(effects.movements);
    for(const row of stock){row.balanceBefore=stockBalance(workspace.domains.food.movements,row.foodId,context.today);row.balanceAfter=stockBalance(after,row.foodId,context.today);}
    const refund=c.operation==='refund',amount=refund?sum(c.entity.lines.map(l=>l.amountPence),'/entity/lines'):c.entity.totalPence;
    const changes={operation:c.operation,purchaseId:c.purchaseId,versionId:c.versionId,previousVersionId:prior.id,date:c.entity.date,merchant:refund?prior.command.entity.merchant.name:c.entity.merchant.name,totalPence:amount,amountPence:amount,beforeTotalPence:prior.command.entity.totalPence,originalTotalPence:prior.command.entity.totalPence,moneyDeltaPence:refund?-amount:amount-prior.command.entity.totalPence,lines:refund?c.entity.lines.map(l=>({...copy(l),description:prior.command.entity.lines.find(p=>p.id===l.lineId).description})):copy(c.entity.lines),adjustments:refund?[]:copy(c.entity.adjustments),stock,priceObservations:refund?0:c.entity.lines.filter(l=>l.productVersionId!==null).length,payment:{mode:refund?'refund':'correct',accountId:account.id,accountName:account.name,date:refund?c.entity.date:c.entity.settlements[0].date,amountPence:amount,transactionId:ids.transactionId,matched:null},reason:refund?c.entity.reason:c.reason};
    return {status:problems.length?'needs-review':'ready',problems,changes,reviewDigest:stable({payloadDigest,revision:context.revision,prior:prior.id,changes,products:[...productMap.values()].filter(p=>!refund&&c.entity.lines.some(l=>l.productVersionId===p.versionId))}),payloadDigest,command:c,recordIds:ids,duplicateCandidates:[],alreadyCommitted:false};
  }
  function amendmentEffects(c,prior,original,ids){
    const prices=[],movements=[];
    if(c.operation==='correct'){
      const oldInventory=prior.command.entity.lines.filter(l=>l.inventory.mode==='receive'),originalInventory=original.command.entity.lines.filter(l=>l.inventory.mode==='receive');let n=0,k=0;
      for(const l of c.entity.lines){if(l.inventory.mode!=='receive')continue;const old=oldInventory[n],grams=l.inventory.grams;
        prices.push({id:ids.foodPurchaseIds[n],receiptId:c.purchaseId,foodId:l.inventory.foodId,date:c.entity.date,store:c.entity.merchant.name,grams,totalPrice:l.netPence/100,totalPricePence:l.netPence,perKg:(l.netPence/100)/grams*1000,attachmentName:c.source.fileName||'',purchaseVersionId:c.versionId,lineId:l.id,productVersionId:l.productVersionId,supersedes:prior.recordIds.foodPurchaseIds[n]});
        const delta=(l.quantity.amount-old.quantity.amount)/1000;if(delta){const anchor=originalInventory.findIndex(p=>p.id===l.id);movements.push({id:ids.movementIds[k++],foodId:l.inventory.foodId,delta,kind:'receipt-correction',referenceId:c.purchaseId,note:c.reason,date:c.entity.date,purchaseVersionId:c.versionId,lineId:l.id,effectiveAfterMovementId:original.recordIds.movementIds[anchor]});}n++;
      }
    }else{let n=0;for(const l of c.entity.lines){if(!l.returnedMilligrams)continue;const old=prior.command.entity.lines.find(p=>p.id===l.lineId);movements.push({id:ids.movementIds[n++],foodId:old.inventory.foodId,delta:-l.returnedMilligrams/1000,kind:'purchase-return',referenceId:c.purchaseId,note:c.entity.reason,date:c.entity.date,purchaseVersionId:c.versionId,lineId:l.lineId});}}
    return {prices,movements};
  }
  function buildAmendment(review,context,workspace){
    const c=review.command,ids=review.recordIds,domain=workspace.domains.purchases,prior=effectiveVersions(domain).find(v=>v.rootId===c.purchaseId),original=domain.versions.find(v=>v.rootId===c.purchaseId&&v.command.operation==='record');
    if(c.operation==='correct')domain.products.push(...copy(c.entity.products));const effects=amendmentEffects(c,prior,original,ids);workspace.domains.food.purchases.push(...effects.prices);workspace.domains.food.movements.push(...effects.movements);
    const event=amendmentMoney(c,workspace,ids,prior);if(event){workspace.domains.money.versions.push(event);workspace.domains.money.sequence=Math.max(workspace.domains.money.sequence,event.sequence);}
    domain.versions.push({id:c.versionId,rootId:c.purchaseId,supersedes:c.operation==='correct'?prior.id:null,operationId:c.operationId,recordedAt:c.source.capturedAt,command:copy(c),recordIds:copy(ids)});
    domain.operations.push({id:c.operationId,payloadDigest:review.payloadDigest,purchaseId:c.purchaseId,versionId:c.versionId,workspaceRevision:context.revision+1,recordIds:copy(ids)});
  ...
  function buildCandidate(command,context){
    ...
    if(c.entity.settlements[0].mode==='create'){const event=makeExpense(c,workspace,ids);workspace.domains.money.versions.push(event);workspace.domains.money.sequence=Math.max(workspace.domains.money.sequence,event.sequence);}
  ...
  function validateDomain(data){try{
    ...
    const roots=new Map(),operations=new Set(),prefix=empty(),productVersions=new Map(data.products.map(p=>[p.versionId,p]));
    for(const v of data.versions){
      fields(v,['id','rootId','supersedes','operationId','recordedAt','command','recordIds'],'/purchases/versions');id(v.rootId,'/purchases/versions/rootId');id(v.operationId,'/purchases/versions/operationId');check(!operations.has(v.operationId),'/purchases/versions','Repeated purchase operation.');operations.add(v.operationId);
      const c=normalize(v.command);check(stable(c)===stable(v.command),'/purchases/versions','Purchase details are not stored in their reviewed canonical shape.');check(c.operationId===v.operationId&&c.purchaseId===v.rootId&&c.versionId===v.id&&v.recordedAt===c.source.capturedAt,'/purchases/versions','Purchase identity or source changed.');const prior=roots.get(v.rootId);if(c.operation==='record')check(v.supersedes===null&&!prior,'/purchases/versions','Repeated original purchase.');else if(c.operation==='correct'){correctionRules(c,prior,prefix);check(v.supersedes===prior.id,'/purchases/versions','Broken purchase correction history.');}else{refundRules(c,prior,prefix);check(v.supersedes===null,'/purchases/versions','A refund cannot replace a purchase.');}check(stable(v.recordIds)===stable(makeRecordIds(c,prior)),'/purchases/versions/recordIds','Purchase record references are inconsistent.');
      ...
      if(c.operation!=='refund')for(const l of c.entity.lines)if(l.productVersionId!==null)check(productVersions.has(l.productVersionId),'/purchases/versions','A recorded line references an absent product version.');
      if(c.operation!=='refund')roots.set(v.rootId,v);prefix.versions.push(v);
    }
  ...
  function requireEffect(rows,expected,path){const actual=rows.find(r=>r.id===expected.id);check(actual,path,'A recorded purchase effect is missing.');for(const key of Object.keys(expected))check(stable(actual[key])===stable(expected[key]),path,'A recorded purchase effect changed: '+key+'.');return actual;}
  function validateAmendmentLinks(v,prior,original,workspace){
    const c=v.command,ids=v.recordIds,d=workspace.domains,effects=amendmentEffects(c,prior,original,ids);
    for(const row of effects.prices)requireEffect(d.food.purchases,row,'/purchases/versions');
    for(const row of effects.movements){const actual=requireEffect(d.food.movements,row,'/purchases/versions');check(!own(actual,'observedGrams'),'/purchases/versions','A purchase change cannot become a stock count.');if(c.operation==='correct'){const anchor=d.food.movements.find(m=>m.id===row.effectiveAfterMovementId);check(anchor&&anchor.kind==='purchase'&&anchor.date===row.date&&anchor.foodId===row.foodId&&d.food.movements.indexOf(anchor)<d.food.movements.indexOf(actual),'/purchases/versions','The receipt correction lost its original stock position.');}else{check(!own(actual,'effectiveAfterMovementId'),'/purchases/versions','Returns use the actual return date and cannot move before a count.');const i=c.entity.lines.findIndex(l=>l.lineId===row.lineId),line=c.entity.lines[i],before=d.food.movements.slice(0,d.food.movements.indexOf(actual)),counts=before.filter(m=>m.foodId===row.foodId&&m.date===row.date&&['count','count-import'].includes(m.kind)),count=counts[counts.length-1];if(count)check(line.order==='after-count'&&line.stockObservationId===count.id&&acknowledged(c,'/entity/lines/'+i+'/returnOrder'),'/purchases/versions','Return stock count ordering evidence is missing or changed.');else check(line.order===undefined&&line.stockObservationId===undefined,'/purchases/versions','Return stock ordering references an absent count.');}}
    if(c.operation==='correct')for(const l of c.entity.lines)if(l.inventory.mode==='receive'){const p=d.purchases.products.find(p=>p.versionId===l.productVersionId);check(p?.ingredientMapping?.foodId===l.inventory.foodId&&p.ingredientMapping.gramsPerBaseUnit===0.001,'/purchases/versions','A corrected stock mapping disagrees with its saved product.');}
    if(!ids.transactionId)return;
    const tx=d.money.versions.find(t=>t.id===ids.transactionId),refund=c.operation==='refund',amount=refund?sum(c.entity.lines.map(l=>l.amountPence),'/entity/lines'):c.entity.totalPence,accountId=refund?c.entity.accountId:c.entity.settlements[0].accountId,day=refund?c.entity.date:c.entity.settlements[0].date;
    check(tx&&tx.rootId===ids.transactionRootId&&tx.accountId===accountId&&tx.date===day&&tx.type===(refund?'refund':'expense')&&tx.amountPence===amount&&tx.amount===amount/100&&stable(tx.legs)===stable([{kind:'accounts',id:accountId,pence:refund?amount:-amount}]),'/purchases/versions','The purchase change payment is missing or inconsistent.');
    if(refund){check(tx.supersedes===null&&tx.refundOfTransactionRootId===prior.recordIds.transactionRootId&&tx.category===d.money.versions.find(t=>t.id===prior.recordIds.transactionId)?.category,'/purchases/versions','Refund payment provenance changed.');}
    else if(ids.transactionId!==prior.recordIds.transactionId){const previous=d.money.versions.find(t=>t.id===prior.recordIds.transactionId);check(tx.supersedes===prior.recordIds.transactionId&&tx.sequence===previous.sequence,'/purchases/versions','The receipt payment correction lost its prior version.');check(tx.category===(receiptCategory(c)===receiptCategory(prior.command)?previous.category:receiptCategory(c))&&tx.note===(c.entity.merchant.name===prior.command.entity.merchant.name?previous.note:c.entity.merchant.name),'/purchases/versions','The corrected payment category or merchant evidence changed.');}
    if(refund||ids.transactionId!==prior.recordIds.transactionId)check(tx.purchaseVersionId===v.id&&tx.operationId==='purchase-payment_'+c.operationId,'/purchases/versions','The purchase change payment lost its provenance.');
  }
  function validateLinks(workspace){try{
    const d=workspace.domains,domain=d.purchases||empty(),valid=validateDomain(domain);check(valid.ok,'/purchases',valid.error);const foods=new Set(d.food.foods.map(f=>f.id)),productMap=new Map(domain.products.map(p=>[p.versionId,p])),usedPayments=new Set(),usedProducts=new Set(),latest=new Map(),originals=new Map();
    ...
    for(const v of domain.versions){
      const c=v.command;if(c.operation!=='record'){const prior=latest.get(v.rootId);validateAmendmentLinks(v,prior,originals.get(v.rootId),workspace);if(c.operation==='correct'){latest.set(v.rootId,v);for(const l of c.entity.lines)if(l.productVersionId!==null)usedProducts.add(l.productVersionId);}continue;}latest.set(v.rootId,v);originals.set(v.rootId,v);const s=c.entity.settlements[0],ids=v.recordIds,tx=d.money.versions.find(t=>t.id===ids.transactionId);check(tx&&tx.rootId===ids.transactionRootId&&tx.accountId===s.accountId&&tx.date===s.date&&tx.type==='expense'&&tx.amountPence===s.amountPence,'/purchases/versions','The recorded payment snapshot is missing or inconsistent.');
  ...
  global.PurchaseOperations=Object.freeze({CONTRACT,SCHEMA,empty,prepare,buildCandidate,validateDomain,validateLinks,stableSerialize:stable,effectiveVersions,refundsFor,refundRemaining,stockBalance});
})(window);
