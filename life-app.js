  ...
  (() => {
    ...
    const nav = [{id:'today',label:'Today',title:'Your day'},{id:'train',label:'Train',title:'Your training'},{id:'plan',label:'Plan',title:'Your programme'},{id:'progress',label:'Progress',title:'Your progress'},{id:'sleep',label:'Sleep',title:'Your sleep'},{id:'food',label:'Food',title:'Your food'},{id:'work',label:'Work',title:'Your work'},{id:'goals',label:'Goals',title:'Your goals'},{id:'life',label:'Life Planner',title:'Your bigger picture'},{id:'money',label:'Money',title:'Your money'},{id:'people',label:'People',title:'Your people'},{id:'capture',label:'Capture',title:'Your check-in'}];
    ...
    function render(){
      ...
      renderNav();
      $('main').innerHTML='<div class="view-enter">'+({today:renderToday,train:renderTrain,plan:renderPlan,progress:renderProgress,sleep:renderSleep,food:renderFood,work:renderWork,goals:renderGoals,money:renderMoney,people:renderPeople,capture:renderCapture,life:renderLifePlanner}[state.route]())+'</div>';
...
const TodayDemo = (() => {
  ...
  function snapshot(date = TODAY) {
    ...
    const moneyNames = {expense: 'Expense', income: 'Income', transfer: 'Transfer', 'debt-payment': 'Debt payment', 'debt-interest': 'Debt interest', valuation: 'Valuation', 'debt-adjustment': 'Debt adjustment'};
    transactions.forEach(t => add('money', t.id, (moneyNames[t.type] || 'Money entry') + (t.category ? ': ' + t.category : ''), amount(t.amount) + (t.accountName ? ' · ' + t.accountName : '') + (t.debtName ? ' · ' + t.debtName : ''), 'recorded', null, ref('money-transaction', t.id)));
    ...
    const moneyPlannedCount = entries.filter(e => e.domain === 'money' && e.status === 'planned').length, expenses = transactions.filter(t => t.type === 'expense');
    summary('money', 'Money', expenses.length ? amount(expenses.reduce((n, t) => n + t.amount, 0)) : transactions.length ? count(transactions.length, 'entry', 'entries') : moneyPlannedCount ? count(moneyPlannedCount, 'item due', 'items due') : 'No entries', expenses.length ? count(expenses.length, 'expense') + ' recorded' + (moneyPlannedCount ? ' · ' + moneyPlannedCount + ' planned' : '') : transactions.length ? 'Money recorded on this date' : moneyPlannedCount ? 'Scheduled amounts are not payments' : 'No money recorded on this date');
    ...
    if (trainingLive) focus = focusOn(trainingLive, 'Your workout is already in progress', 'Continue workout');
    ...
    else if (isToday && trainingPlan) focus = focusOn(trainingPlan, 'A workout is scheduled for today and has no completed session', 'Start workout');
    else if (!isPast) {
      ...
      if (commitments.length) focus = focusOn(commitments[0], 'The next saved time on the selected date', 'Open plan');
      if (!focus) {
...
function todayPanorama(snapshot) {
  ...
  return '<section class="today-panorama" aria-label="Your life, connected"><div class="today-panorama-heading"><h2>Your life, connected</h2><p>Open a strand to go deeper.</p></div><div class="today-panorama-field">'+
    ...
    function suggestion(p){
      const e=p.exercise||exercise(p.exerciseId);const prev=previousExercise(e.id);
      if(!prev||prev.sets.length<p.targetSets)return {weight:e.base,reps:p.minReps,message:'Build a baseline. Choose a comfortable load within your rep range.'};
      const weight=prev.sets[0].weight;const consistent=prev.sets.every(s=>s.weight===weight);const atCeiling=prev.sets.every(s=>s.reps>=p.maxReps);
      if(consistent&&atCeiling)return {weight:Number((weight+e.step).toFixed(2)),reps:p.minReps,message:'All '+prev.sets.length+' previous sets reached '+p.maxReps+' reps. Try '+Number((weight+e.step).toFixed(2))+' '+e.unit+' at '+p.minReps+' reps.'};
      return {weight,reps:Math.min(p.maxReps,Math.max(p.minReps,prev.sets[0].reps)),message:'Keep '+weight+' '+e.unit+'. Aim for one extra rep across the session before adding load.'};
    }
    ...
    function showDialog(title,html){dialogOpener=document.activeElement;$('dialogContent').innerHTML='<div class="dialog-header"><h2 id="dialogTitle">'+esc(title)+'</h2><button class="icon-button" data-action="close-dialog" aria-label="Close dialog">'+icon('close')+'</button></div>'+html;if(!$('dialog').open)$('dialog').showModal();}
    ...
    function sleepTimestamp(d,t){
      if(!sleepDateValid(d)||!/^([01]\d|2[0-3]):[0-5]\d$/.test(t))return null;
      const stamp=new Date(d+'T'+t+':00');
      if(stamp.getFullYear()!==Number(d.slice(0,4))||stamp.getMonth()+1!==Number(d.slice(5,7))||stamp.getDate()!==Number(d.slice(8,10))||stamp.getHours()!==Number(t.slice(0,2))||stamp.getMinutes()!==Number(t.slice(3,5)))return null;
    ...
    function validateSleep(value,records,editingId){
      ...
      let timeInBed=null;
      if(value.bedDate!==null){const start=sleepTimestamp(value.bedDate,value.bedTime||''),end=sleepTimestamp(value.wakeDate,value.wakeTime||'');if(start===null||end===null)return {error:'Add valid bed and wake times. A time skipped by a clock change cannot be used.'};timeInBed=Math.round((end-start)/60000);if(timeInBed<=0||timeInBed>1440)return {error:'Got-up time must be after bedtime, within 24 hours. Check both dates.'};if(value.duration>timeInBed)return {error:'Time asleep cannot be longer than time in bed ('+sleepDuration(timeInBed)+').'};}
      if(value.kind==='main'){const duplicate=records.find(r=>r.kind==='main'&&r.wakeDate===value.wakeDate&&r.id!==editingId);if(duplicate)return {error:'Main sleep is already recorded for this wake date. Correct that entry or choose another date.',duplicate:duplicate.id};}
      return {timeInBed};
    }
    function updateSleepTimePreview(){if(!$('sleepForm'))return;const v=sleepFormValues(),start=sleepTimestamp($('sleepBedDate').value,$('sleepBedTime').value),end=sleepTimestamp(v.wakeDate,$('sleepWakeTime').value);$('sleepTimeOutcome').textContent=start!==null&&end!==null&&end>start&&end-start<=86400000?'Time in bed: '+sleepDuration(Math.round((end-start)/60000))+' · '+dateLabel($('sleepBedDate').value)+' to '+dateLabel(v.wakeDate):'Choose bed and wake dates and times to see time in bed.';}
    function saveSleep(event){event.preventDefault();const v=sleepFormValues(),result=validateSleep(v,activeSleepRecords(),sleepEditingId);$('sleepError').textContent=result.error||'';$('sleepDuplicate').innerHTML=result.duplicate?'<button type="button" class="text-button" data-action="sleep-edit" data-id="'+result.duplicate+'">Open the existing entry '+icon('arrow')+'</button>':'';if(result.error)return;
      ...
      const record={...v,id:uid('sleep-version'),sessionId:prior?prior.sessionId:uid('sleep-session'),timeInBed:result.timeInBed,source:'Manual entry',zone:sleepZone,targetSnapshot:v.kind==='main'?(historicalMain?historicalMain.targetSnapshot:sleepView.goal):null,createdAt:new Date().toISOString(),supersedes:prior?prior.id:null};sleepRevisions.push(record);sleepView.selected=v.wakeDate;sleepView.end=v.wakeDate;sleepView.range=7;sleepView.page=0;closeDialog();navigate('sleep');toast(prior?'Correction saved. The earlier version is retained in this app.':'Sleep added to your record.');
    }
...
const FoodDemo = (() => {
  ...
  function stock(id) {
    return rounded(movements.filter(m=>m.foodId===id).reduce((total,m)=>total+m.delta,0));
  }
  ...
  function prices(id) {
    return purchases.filter(p=>p.foodId===id).slice().sort((a,b)=>a.date.localeCompare(b.date)||a.id.localeCompare(b.id));
  }
  ...
  function setStock(foodId,grams,note) {
    if(!food(foodId)) return fail('Choose a food from the library.');
    if(!mass(grams)) return fail('Stock must be a finite amount of zero grams or more.');
    movement(foodId,grams-stock(foodId),'count',null,note||'Stock counted');
    return {ok:true,grams};
  }
  function importStock(rows) {
    if(!Array.isArray(rows)||rows.length===0) return fail('Add at least one stock row.');
    const seen=new Set();
    for(const row of rows) {
      if(!row||!food(row.foodId)||!mass(row.grams)) return fail('Every row needs a known food and a non-negative quantity in grams.');
      if(seen.has(row.foodId)) return fail('Each food can appear only once in a stock count.');
      seen.add(row.foodId);
    }
    rows.forEach(row=>movement(row.foodId,row.grams-stock(row.foodId),'count-import',null,'Opening stock count'));
  ...
  function addReceipt(receipt) {
    ...
    for(const line of receipt.lines) {
      if(!line||!food(line.foodId)||!positive(line.grams)||!mass(line.totalPrice)) return fail('Every receipt line needs a known food, positive grams and the total price paid.');
    }
    ...
    const entries=receipt.lines.map(line=>({id:uid('purchase'),receiptId,foodId:line.foodId,date:receipt.date,store:storeName,grams:line.grams,totalPrice:line.totalPrice,perKg:line.totalPrice/line.grams*1000,attachmentName:typeof receipt.attachmentName==='string'?receipt.attachmentName:''}));
  ...
  function restore(data){
    ...
    try{
      ...
      if(next.movements.some(m=>!foodIds.has(m.foodId)||!Number.isFinite(m.delta)||!validDate(m.date)||typeof m.kind!=='string'))throw Error();
      if(next.purchases.some(p=>!foodIds.has(p.foodId)||!validDate(p.date)||!positive(p.grams)||!mass(p.totalPrice)||!mass(p.perKg)||typeof p.store!=='string'))throw Error();
...
function goalEditor(id=null) {
  ...
  const MoneyDemo = (() => {
    ...
    function month(key) {
      if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(key)) return { income: 0, spending: 0, transfers: 0, savingsAdded: 0, investmentAdded: 0, debtPaid: 0, byCategory: [], transactions: [] };
      const events = latest().filter(event => event.date.slice(0, 7) === key);
      const sumType = type => pounds(events.filter(event => event.type === type).reduce((sum, event) => sum + event.amountPence, 0));
      ...
      const categories = {};
      events.filter(event => event.type === 'expense').forEach(event => { categories[event.category] = (categories[event.category] || 0) + event.amountPence; });
      return { income: sumType('income'), spending: sumType('expense'), transfers: sumType('transfer'), savingsAdded: netTransfers('savings'), investmentAdded: netTransfers('investment'), debtPaid: sumType('debt-payment'), byCategory: Object.entries(categories).map(([category, amount]) => ({ category, amount: pounds(amount) })).sort((a, b) => b.amount - a.amount), transactions: copy(events.slice().reverse()) };
    }
    ...
    function restore(data){return LifeSnapshotCheck.protect(()=>{
      ...
      const current=C.chain(C.list(s.versions,'money versions'),event=>{
        C.date(event.date,true);C.check(event.sequence<=s.sequence,'Journal sequence is behind its events.');C.one(event.type,['income','expense','transfer','debt-payment','debt-interest','valuation','debt-adjustment']);C.integer(event.amountPence,1);C.money(event.amount);C.check(event.amount===event.amountPence/100,'Money display and integer amount disagree.');C.text(event.category,80);C.text(event.note,1000);for(const key of ['accountName','toAccountName','debtName'])C.text(event[key],80);
    ...
    const api = {
      ...
      month, netWorth, history, projectDebt, projectContributions
    };
    return api;
  })();
...
const MoneyReferenceDemo = (() => {
  ...
  return Object.freeze({
    ...
    restore(data){return LifeSnapshotCheck.protect(()=>{const C=LifeSnapshotCheck,s=C.payload(data,'lifeos.money-reference-state.v1');if(s.reference===null){currentReference=null;return {ok:true};}const r=s.reference;C.object(r);C.check(r.schema===SCHEMA,'Unsupported reference schema.');C.text(r.fileName,180,true);C.optional(r.asOf,C.date);C.check(Array.isArray(r.items)&&r.items.length>0&&r.items.length<=MAX_ITEMS,'Invalid reference items.');C.indexed(r.items,'reference items');for(const row of r.items){C.one(row.kind,[...KINDS]);C.text(row.name,300,true);C.text(row.note,1400);C.check(row.reviewStatus==='needs-checking','Imported reference must remain unverified.');for(const key of ['amount','apr','minPayment'])C.optional(row[key],v=>C.number(v,0,key==='apr'?1000:1e12));}C.check(Array.isArray(r.notes)&&r.notes.length<=105,'Invalid reference notes.');r.notes.forEach(v=>C.text(v,1400));currentReference=freezeReference(r);return {ok:true};});},
    ...
    const moneyMovementAmount=t=>['valuation','debt-adjustment'].includes(t.type)?(t.adjustment>0?'+':'')+moneyFormat(t.adjustment):(t.type==='income'?'+':t.type==='expense'||t.type==='debt-payment'?'−':'')+moneyFormat(t.amount);
    ...
    const moneyTxType=t=>({'income':'Income','expense':'Spending','transfer':'Account transfer','debt-payment':'Debt payment','debt-interest':'Interest added','valuation':'Balance adjustment','debt-adjustment':'Debt adjustment'}[t]||t);
    ...
    function moneyTransactionDetail(id){const t=MoneyDemo.transactions.find(t=>t.id===id);if(!t)return;const versions=MoneyDemo.transactionVersions.filter(v=>(v.rootId||v.id)===(t.rootId||t.id)).slice().reverse();showDialog(moneyTxType(t.type),'<p class="dialog-sub">'+dateLabel(t.date,{day:'numeric',month:'long',year:'numeric'})+' · '+esc(t.accountName||t.debtName||'Balance record')+'</p><div class="money-number">'+moneyMovementAmount(t)+'</div><p class="money-note">'+esc(t.note||(['income','expense'].includes(t.type)?t.category:null)||'No note')+(t.toAccountName?'<br>To '+esc(t.toAccountName):'')+(t.debtName&&t.accountName?'<br>Debt: '+esc(t.debtName):'')+'</p>'+(['income','expense','transfer','debt-payment','debt-interest'].includes(t.type)?'<button class="text-button" data-action="money-transaction-edit" data-id="'+t.id+'">Correct entry '+icon('edit')+'</button>':'<p class="money-note">To revise a balance check, record a new current balance from the account or debt.</p>')+'<h3 class="gap-top" style="font-size:14px">Record history</h3>'+versions.map((v,i)=>'<div class="work-revision"><strong>'+(i===0?'Current record':'Earlier version')+'</strong><br>'+dateLabel(v.date)+' · '+moneyTxType(v.type)+' · '+moneyMovementAmount(v)+'<br>'+esc(v.note||'')+'</div>').join('')+'<div class="dialog-footer"><button class="button primary" data-action="close-dialog">Done</button></div>');}
    ...
    function restoreMoneySetup(data){return LifeSnapshotCheck.protect(()=>{const C=LifeSnapshotCheck,s=C.payload(data,'lifeos.money-setup.v1');C.object(s.allowances);for(const key of ['essentials','flexible','extraDebt','savings','investment'])C.money(s.allowances[key]);moneySetup.allowances=s.allowances;moneyView.extra=MoneyDemo.plan.extraDebt;return {ok:true};});}
  ...
  function apply(proposal, options = {}) {
    ...
    try {
      ...
      if (target === 'sleep') {
        ...
        const historicalMain = prior ? sleepRevisions.filter(r => r.sessionId === prior.sessionId && r.kind === 'main' && r.targetSnapshot !== null).slice(-1)[0] : null;
        const record = { ...value, id: uid('sleep-version'), sessionId: prior ? prior.sessionId : uid('sleep-session'), timeInBed: checked.timeInBed, source: 'Capture review', zone: sleepZone, targetSnapshot: value.kind === 'main' ? historicalMain ? historicalMain.targetSnapshot : sleepView.goal : null, createdAt: new Date().toISOString(), supersedes: prior ? prior.id : null, operationId: op };
...
function renderCaptureMap(summary,selected='all') {
  ...
  return '<section class="capture-map" aria-label="Capture destinations"><div class="capture-map-header"><div><span class="eyebrow">Across your life</span><h2>One thought, connected.</h2></div><p>'+(totals.total?overview:'Your words can reach the right places.')+'</p></div><div class="capture-map-canvas">'+
    ...
    function captureAction(a,d){
      ...
      if(a==='capture-commit'){const r=CaptureDemo.commit();captureRender();const saved=r.results?.filter(x=>x.status==='saved').length??0;toast(saved?saved+' '+(saved===1?'entry added.':'entries added.'):'Check the review for anything that still needs attention.');return;}
    ...
    let foodReceiptDraft=null;
    function openFoodReceipt(){foodReceiptDraft={token:uid('receipt-draft'),date:TODAY,store:'',attachmentName:'',imageData:null,lines:[{foodId:'',grams:'',totalPrice:''}]};renderFoodReceipt();}
    function syncFoodReceipt(){if(!$('foodReceiptForm'))return;foodReceiptDraft.date=$('foodReceiptDate').value;foodReceiptDraft.store=$('foodReceiptStore').value;foodReceiptDraft.lines=Array.from(document.querySelectorAll('.food-receipt-row')).map(row=>({foodId:row.querySelector('[data-receipt="food"]').value,grams:row.querySelector('[data-receipt="grams"]').value,totalPrice:row.querySelector('[data-receipt="price"]').value}));}
    function renderFoodReceipt(){const d=foodReceiptDraft;showDialog('Review a food purchase','<form id="foodReceiptForm"><p class="dialog-sub">Confirm the foods, quantities and prices before adding anything to the pantry.</p><div class="food-grid"><label>Purchase date<input id="foodReceiptDate" type="date" max="'+TODAY+'" value="'+d.date+'" required></label><label>Shop<input id="foodReceiptStore" value="'+esc(d.store)+'" placeholder="Shop name" maxlength="100" required></label></div><div class="food-receipt-preview"><label>Attach receipt image, optional<input id="foodReceiptFile" type="file" accept="image/png,image/jpeg,image/webp"></label><p class="gap-sm">The image is shown only while you enter this receipt and is not kept. Saved purchase rows retain the quantities, prices and filename. Enter and check the rows below.</p><div id="foodReceiptImage">'+(d.imageData?'<img src="'+d.imageData+'" alt="Your attached receipt">':'')+'</div><p id="foodReceiptFilename">'+esc(d.attachmentName)+'</p></div><div id="foodReceiptRows">'+d.lines.map((line,i)=>'<div class="food-receipt-row"><div class="row"><label style="flex:1;min-width:0"><span class="sr">Purchased food '+(i+1)+'</span><select data-receipt="food" required>'+foodOptions(line.foodId)+'</select></label><button class="icon-button" type="button" data-action="food-receipt-remove" data-index="'+i+'" aria-label="Remove purchase line '+(i+1)+'" '+(d.lines.length===1?'disabled':'')+'>'+icon('trash')+'</button></div><div class="food-grid"><label>Total quantity, grams<input data-receipt="grams" type="number" min="0.1" max="1000000" step="any" inputmode="decimal" value="'+esc(line.grams)+'" required></label><label>Total price paid, £<input data-receipt="price" type="number" min="0" max="100000" step="0.01" inputmode="decimal" value="'+esc(line.totalPrice)+'" required></label></div><p>Use the total quantity bought across all packs. Missing weights need a manual entry.</p></div>').join('')+'</div><button type="button" class="text-button" data-action="food-receipt-add">+ Add purchase line</button><div class="food-preview-total"><span>Total entered</span><strong id="foodReceiptTotal">'+foodMoney(d.lines.reduce((n,l)=>n+(Number(l.totalPrice)||0),0))+'</strong></div><div id="foodReceiptError" class="error" role="alert"></div><div class="dialog-footer"><button type="button" class="button ghost" data-action="close-dialog">Cancel</button><button type="submit" class="button primary">Confirm purchase</button></div></form>');}
    function renderFoodPrices(){const f=FoodDemo.food(foodView.priceFood)||FoodDemo.foods[0];if(!f)return '<div class="food-empty"><h3>No foods in your library.</h3><p>Add your food library before recording purchases or comparing prices.</p></div>';foodView.priceFood=f.id;const all=FoodDemo.prices(f.id).slice().sort((a,b)=>a.date.localeCompare(b.date)),rows=all.filter(p=>p.date<=TODAY&&(!foodView.priceRange||p.date>=addDays(TODAY,-foodView.priceRange+1))),latest=rows[rows.length-1],first=rows[0];const selected=rows.find(r=>r.id===foodView.priceId)||latest;
    ...
    function handleFoodAction(a,d){
      ...
      if(a==='food-receipt-add'){syncFoodReceipt();foodReceiptDraft.lines.push({foodId:'',grams:'',totalPrice:''});renderFoodReceipt();return;}
      if(a==='food-receipt-remove'){syncFoodReceipt();foodReceiptDraft.lines.splice(Number(d.index),1);renderFoodReceipt();return;}
    ...
    document.addEventListener('input',event=>{
      ...
      if(event.target.closest('.food-receipt-row')){syncFoodReceipt();$('foodReceiptTotal').textContent=foodMoney(foodReceiptDraft.lines.reduce((n,l)=>n+(Number(l.totalPrice)||0),0));}
    ...
    document.addEventListener('change',event=>{
      ...
      if(event.target.id==='foodReceiptFile'){const file=event.target.files[0],draft=foodReceiptDraft;if(!file)return;if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>8*1024*1024){$('foodReceiptError').textContent='Choose a PNG, JPEG or WebP image under 8 MB.';return;}const reader=new FileReader();reader.onload=()=>{if(foodReceiptDraft!==draft||!$('foodReceiptForm'))return;draft.imageData=reader.result;draft.attachmentName=file.name;$('foodReceiptImage').innerHTML='<img src="'+draft.imageData+'" alt="Your attached receipt">';$('foodReceiptFilename').textContent=file.name;};reader.onerror=()=>{if($('foodReceiptError'))$('foodReceiptError').textContent='The image could not be read. You can still enter the purchase below.';};reader.readAsDataURL(file);}
    ...
    document.addEventListener('submit',event=>{
      ...
      else if(id==='foodStockForm'){result=FoodDemo.setStock(event.target.dataset.id,Number($('foodStockGrams').value),$('foodStockNote').value);if(!result.ok){$('foodStockError').textContent=result.error;return;}}
      else if(id==='foodReceiptForm'){syncFoodReceipt();result=FoodDemo.addReceipt({...foodReceiptDraft,lines:foodReceiptDraft.lines.map(l=>({...l,grams:Number(l.grams),totalPrice:Number(l.totalPrice)}))});if(!result.ok){$('foodReceiptError').textContent=result.error;return;}foodView.tab='pantry';foodView.query='';foodView.filter='all';}
    ...
    document.addEventListener('click',event=>{
      ...
      if(a.startsWith('workspace-')){workspaceAction(a).catch(error=>showToast(error.message));return;}
      ...
      if(a.startsWith('capture-')){captureAction(a,d);return;}
...
document.addEventListener('submit',event=>{
  ...
  if(!lifeResult(result,errorId))return;closeDialog();render();toast(message);
    ...
    function workspaceParts(){return {
      ...
      captureReceipts:{snapshot:()=>CaptureTargetsDemo.persistenceSnapshot(),restore:data=>CaptureTargetsDemo.restore(data)}
    };}
    function captureWorkspace(){const domains={};for(const[name,part]of Object.entries(workspaceParts()))domains[name]=part.snapshot();return {format:'lifeos-state/1',domains,drafts:{capture:{draft:captureView.draft,date:captureView.date,source:captureView.source,fileName:captureView.fileName},ambitions:clone(lifeView.drafts)}};}
function validateWorkspaceLinks(payload) {
  ...
  return { ok: true };
    ...
    function restoreWorkspace(payload){
      ...
      validateWorkspaceLinks(payload);
      ...
      try{for(const[name,part]of Object.entries(parts)){const result=part.restore(payload.domains[name]);if(!result||!result.ok)throw new Error('Could not restore '+name+': '+(result?.error||'invalid records'));}
        ...
        captureView.draft=capture.draft||'';captureView.date=todayAdapterDate(capture.date)||TODAY;captureView.source=['typed','file','plaud','paste','manual'].includes(capture.source)?capture.source:'typed';captureView.fileName=typeof capture.fileName==='string'?capture.fileName.slice(0,250):'';
        lifeView.drafts=clone(ambitionDrafts);lifeView.id=null;goalView.id=null;
    ...
    function workspaceSettings(){showDialog('Your private workspace','<div class="workspace-settings"><span class="eyebrow">Life OS v60</span><p class="dialog-sub">Your records are saved in this browser on this device. No account, data upload or analytics is connected.</p><div class="workspace-save-info"><strong>'+esc(LifeOSRuntime.error?'A save needs attention':LifeOSRuntime.saving?'Saving your changes':'Saved on this device')+'</strong><p>'+esc(LifeOSRuntime.error?.message||'Use encrypted backups to keep a recoverable copy under your control.')+'</p>'+(LifeOSRuntime.error?'<button class="button primary" data-action="workspace-retry">Try saving again</button>':'')+'</div><div class="workspace-backup-actions"><button class="button primary" data-action="workspace-export">'+icon('shield')+' Create encrypted backup</button><button class="button ghost" data-action="workspace-import">'+icon('upload')+' Restore a backup</button></div><h3>Keep your record recoverable</h3><p>Your phone\'s encryption and screen lock protect the device. The browser database has no separate app password. A backup is encrypted before it is downloaded; only you choose where to keep that file.</p><p>Keep the backup password somewhere safe. Life OS cannot recover it. Clearing this site\'s storage removes the local records.</p><div id="workspaceCapacity" class="workspace-capacity">Checking local storage...</div><button class="text-button" data-action="workspace-persist">Request persistent device storage</button><h3>Connected services</h3><p>Text capture works locally. Sleep, purchases and finances can be entered manually. Garmin, automatic receipt recognition, private voice processing and background reminders are not connected in this release.</p><details><summary>Records from the previous app</summary><p>The previous app\'s local records are left untouched. Encrypted backups include an archive of them. The redesigned workspace starts clean; old records have not been silently converted into the new journal formats.</p></details><div class="dialog-footer"><button class="button ghost" data-action="workspace-update">Check for app updates</button><button class="button primary" data-action="close-dialog">Done</button></div></div>');
    ...
    function workspaceBackupDialog(){showDialog('Create an encrypted backup','<form id="workspaceBackupForm"><p class="dialog-sub">All your sections, their history and any previous-app archive are included. The file is encrypted on this device.</p><label>Backup password<input id="backupPassword" type="password" minlength="12" maxlength="1000" autocomplete="new-password" required></label><label class="gap-top">Repeat password<input id="backupPasswordAgain" type="password" minlength="12" maxlength="1000" autocomplete="new-password" required></label><p class="workspace-form-help">Use at least 12 characters. Keep it safe: there is no password reset for an encrypted file.</p><div id="backupError" class="error" role="alert"></div><div class="dialog-footer"><button type="button" class="button ghost" data-action="close-dialog">Cancel</button><button class="button primary" type="submit">Encrypt and download</button></div></form>');}
    function workspaceImportDialog(){pendingBackup=null;showDialog('Restore an encrypted backup','<form id="workspaceImportForm"><p class="dialog-sub">First decrypt and review the file. Your current workspace stays in place until you choose to restore it.</p><label>Life OS backup<input id="backupFile" type="file" accept=".lifeos,.json,application/json" required></label><label class="gap-top">Backup password<input id="restorePassword" type="password" maxlength="1000" autocomplete="off" required></label><div id="restoreError" class="error" role="alert"></div><div class="dialog-footer"><button type="button" class="button ghost" data-action="close-dialog">Cancel</button><button class="button primary" type="submit">Decrypt and review</button></div></form>');}
    async function workspaceSubmit(event){const form=event.target;if(!['workspaceBackupForm','workspaceImportForm'].includes(form.id))return;event.preventDefault();if(backupBusy)return;backupBusy=true;const button=form.querySelector('button[type="submit"]'),error=$(form.id==='workspaceBackupForm'?'backupError':'restoreError');if(button)button.disabled=true;error.textContent='';
      try{if(form.id==='workspaceBackupForm'){const password=$('backupPassword').value;if(password!==$('backupPasswordAgain').value)throw new Error('The two passwords do not match.');const payload=LifeOSRuntime.backupPayload(),encrypted=await LifeOSVault.encrypt(payload,password),blob=new Blob([JSON.stringify(encrypted)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='lifeos-'+TODAY+'.lifeos';document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);closeDialog();showToast('Encrypted backup created. Check that the file has downloaded.');}
        else{const file=$('backupFile').files[0];if(!file)throw new Error('Choose a backup file.');if(file.size>LifeOSVault.LIMIT*1.4)throw new Error('This backup exceeds the supported file size.');const envelope=JSON.parse(await file.text()),decoded=await LifeOSVault.decrypt(envelope,$('restorePassword').value);if(decoded.format!=='lifeos-backup/2'||!decoded.workspace)throw new Error('This is not a supported workspace backup.');pendingBackup=decoded;showDialog('Review the restore','<p class="dialog-sub">Backup created '+esc(decoded.exportedAt||'on an unknown date')+'.</p><p>This replaces the current redesigned workspace with the records in this backup. It includes all sections, history and any running clocks saved at that time.</p><p class="gap-top">Create a backup of your current workspace first if you want to keep both versions. Previous-app local data remains untouched.</p><div id="restoreCommitError" class="error" role="alert"></div><div class="dialog-footer"><button class="button ghost" data-action="close-dialog">Keep current records</button><button class="button primary" data-action="workspace-restore-confirm">Restore these records</button></div>');}
    ...
    async function workspaceAction(action){if(action==='workspace-settings'){workspaceSettings();return;}if(action==='workspace-export'){workspaceBackupDialog();return;}if(action==='workspace-import'){workspaceImportDialog();return;}
      if(action==='workspace-retry'){try{await LifeOSRuntime.retry();workspaceSettings();}catch(error){workspaceSettings();}return;}
      ...
      if(action==='workspace-restore-confirm'&&pendingBackup&&!backupBusy){backupBusy=true;try{await LifeOSRuntime.restoreBackup(pendingBackup);pendingBackup=null;closeDialog();render();showToast('Your backup has been restored.');}catch(error){if($('restoreCommitError'))$('restoreCommitError').textContent=error.message;}finally{backupBusy=false;}}
    }
    ...
    async function registerOffline(){if(!('serviceWorker'in navigator))return;try{const registration=await navigator.serviceWorker.register('./sw.js',{updateViaCache:'none'});let acceptedHere=false;const reloadWhenSafe=async()=>{if(LifeOSRuntime.restoring||backupBusy){showToast('Finish the backup operation before opening the update.');return;}try{await LifeOSRuntime.flush();window.location.reload();}catch(error){workspaceStatus('error',error.message);}};const showUpdate=()=>{if(registration.waiting){$('updateButton').hidden=false;$('updateButton').textContent='Update ready';$('updateButton').onclick=async()=>{try{if(LifeOSRuntime.restoring||backupBusy){showToast('Finish the backup operation before updating.');return;}await LifeOSRuntime.flush();acceptedHere=true;registration.waiting?.postMessage({type:'SKIP_WAITING'});}catch(error){workspaceStatus('error',error.message);}};}};showUpdate();registration.addEventListener('updatefound',()=>{registration.installing?.addEventListener('statechange',showUpdate);});let hadController=!!navigator.serviceWorker.controller;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(hadController){$('updateButton').hidden=false;$('updateButton').textContent='Open new version';$('updateButton').onclick=reloadWhenSafe;if(acceptedHere)reloadWhenSafe();}hadController=true;});await registration.update();}catch(error){if(!LifeOSRuntime.error)workspaceStatus('offline-warning',navigator.serviceWorker.controller?'Saved locally. Update check unavailable while offline.':'Saved locally. Offline installation needs an online retry.');}}
    async function bootWorkspace(){try{await LifeOSRuntime.start({snapshot:captureWorkspace,restore:restoreWorkspace,render,status:workspaceStatus});render();registerOffline();document.addEventListener('visibilitychange',()=>{if(!document.hidden)refreshWorkspaceDay();});setInterval(refreshWorkspaceDay,30000);setInterval(updateTimer,250);setInterval(()=>{updateWorkLive();updateTodayLive();},1000);}catch(error){workspaceStatus('error',error.message);$('main').innerHTML='<section class="workspace-open-error"><h1>Your records stay in place.</h1><p>'+esc(error.message)+'</p><p>Life OS could not safely open the workspace. No replacement records have been saved.</p><button class="button primary" id="retryOpenWorkspace">Try opening again</button></section>';$('retryOpenWorkspace').onclick=()=>location.reload();}}
    for(const type of ['click','input','change','submit','keydown'])document.addEventListener(type,event=>{if(LifeOSRuntime.restoring||(!LifeOSRuntime.ready&&event.target?.id!=='retryOpenWorkspace')){event.preventDefault();event.stopImmediatePropagation();}},true);
    ...
    window.LifeOSApp=Object.freeze({get version(){return 'v60';},snapshot:captureWorkspace,restore:async data=>LifeOSRuntime.restoreBackup({format:'lifeos-backup/2',workspace:data}),domains:()=>workspaceParts(),capture:CaptureDemo,captureTargets:CaptureTargetsDemo,save:()=>LifeOSRuntime.flush()});
    ...
    window.addEventListener('hashchange',()=>{const route=window.location.hash.slice(1);if(nav.some(n=>n.id===route)&&route!==state.route){state.route=route;render();window.scrollTo({top:0,behavior:'instant'});}});
    bootWorkspace();
  })();
