  ...
  (() => {
    ...
    const nav = [{id:'today',label:'Today',title:'Your day'},{id:'train',label:'Train',title:'Your training'},{id:'plan',label:'Programme',title:'Your programme'},{id:'progress',label:'Progress',title:'Your progress'},{id:'sleep',label:'Sleep',title:'Your sleep'},{id:'food',label:'Food',title:'Your food'},{id:'work',label:'Work',title:'Your work'},{id:'goals',label:'Goals',title:'Your goals'},{id:'life',label:'Life Planner',title:'Your bigger picture'},{id:'money',label:'Money',title:'Your money'},{id:'people',label:'People',title:'Your people'},{id:'capture',label:'Capture',title:'Your check-in'}];
...
const FoodDemo = (() => {
  ...
  function stock(id,date=TODAY) {
    ...
    // Date is effective time; entries on the same date retain their recording order.
    const ordered=movements.map((entry,index)=>({entry,index})).filter(row=>row.entry.foodId===id&&row.entry.date<=date).sort((a,b)=>a.entry.date.localeCompare(b.entry.date)||a.index-b.index);
...
document.addEventListener('submit',event=>{
  ...
  if(!lifeResult(result,errorId))return;closeDialog();render();toast(message);
    ...
    function workspaceParts(){return {
      ...
      captureReceipts:{snapshot:()=>CaptureTargetsDemo.persistenceSnapshot(),restore:(data,options)=>CaptureTargetsDemo.restore(data,options)}
    };}
    function captureWorkspace(){const domains={};for(const[name,part]of Object.entries(workspaceParts()))domains[name]=part.snapshot();return {format:'lifeos-state/1',minimumReaderVersion:62,domains,drafts:{capture:{draft:captureView.draft,date:captureView.date,source:captureView.source,fileName:captureView.fileName},ambitions:clone(lifeView.drafts)}};}
function validateWorkspaceLinks(payload){return LifeOSWorkspace.validateLinks(payload);}
    ...
    window.LifeOSApp=Object.freeze({get version(){return 'v62';},snapshot:captureWorkspace,restore:async data=>LifeOSRuntime.restoreBackup({format:'lifeos-backup/2',workspace:data}),domains:()=>workspaceParts(),capture:CaptureDemo,captureTargets:CaptureTargetsDemo,commitCapture:commitCaptureDurably,save:()=>LifeOSRuntime.flush()});
