...
document.addEventListener('submit',event=>{
  ...
  if(!lifeResult(result,errorId))return;closeDialog();render();toast(message);
    ...
    function workspaceParts(){return {
      ...
      captureReceipts:{snapshot:()=>CaptureTargetsDemo.persistenceSnapshot(),restore:(data,options)=>CaptureTargetsDemo.restore(data,options)},purchases:PurchasesDemo
    };}
    function captureWorkspace(){const domains={};for(const[name,part]of Object.entries(workspaceParts()))domains[name]=part.snapshot();return {format:'lifeos-state/1',minimumReaderVersion:63,domains,drafts:{capture:{draft:captureView.draft,date:captureView.date,source:captureView.source,fileName:captureView.fileName},ambitions:clone(lifeView.drafts),receipt:PurchaseUI.snapshotDraft()}};}
function validateWorkspaceLinks(payload){return LifeOSWorkspace.validateLinks(payload);}
    ...
    window.LifeOSApp=Object.freeze({get version(){return 'v63';},snapshot:captureWorkspace,restore:async data=>LifeOSRuntime.restoreBackup({format:'lifeos-backup/2',workspace:data}),domains:()=>workspaceParts(),capture:CaptureDemo,captureTargets:CaptureTargetsDemo,commitCapture:commitCaptureDurably,preparePurchase,commitPurchase,save:()=>LifeOSRuntime.flush()});
