...
(function (global) {
  ...
  // mealTemplates: the meal library, live and editable, never history. A
  // template either states components ([{foodId, grams}] PER SINGLE SERVING,
  // matching the sheet's gramsPerServing) and derives its nutrition from the
  // food library, or has none and keeps the hand-entered numbers it was built
  // with. Either way the values land in the same calories, protein, carbs, fat
  // and sugar fields, so a reader never has to know which kind it holds, and
  // nutritionSource ("components"|"manual") says which it was. Derived values
  // are a snapshot taken at write time, not a live link: editing a food moves
  // a template only when that template is next written. An optional field
  // (satFat, fibre, salt) is derived only when EVERY component states it,
  // because absent means unknown and summing an unknown as zero would invent a
  // reading. ids are meal_<slug>, enforced by LifeOSWrite.mealTemplate.
  "mealTemplates",

  "mealPlans",

  // dietLogs: one record per day, id diet_<date>. ITEMS ARE THE TRUTH. totals
  // are derived from items on every write by LifeOSWrite.dietLog and a totals
  // passed in by a caller is ignored, which is what stops quick-log and the
  // Log tab from disagreeing about whether logging accumulates or replaces.
  // The two diet metrics (diet_calories_kcal, diet_protein_g) are a projection
  // of these totals and are emitted by that writer, not by callers.
  //
  // A logged item is IMMUTABLE HISTORY. It carries snapshot.perServing, what
  // one serving of that meal was worth at the moment it was logged, plus
  // snapshot.at, .name and .source ("components"|"manual"|"custom"). Totals
  // apply the item's servings to it, so correcting a miscount rescales frozen
  // numbers rather than re-reading the library. Editing a food or a template
  // afterwards can never move a meal already eaten.
  //
  // An item with NO snapshot key predates snapshots. It is never backfilled,
  // not even when edited, because a snapshot invented today would be a
  // retroactive rewrite; it is resolved live instead and reported as legacy.
  // For the same reason, a write that leaves such a day's items deeply
  // unchanged keeps the stored totals verbatim and touches no metric: a
  // recorded total is itself a fact. Change an item and derivation resumes.
  "dietLogs",
