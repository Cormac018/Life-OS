...
if (!next.dietPrepNotesV1) {
  ...
  function upsertTemplate(t) {
    // Canonical writer. It enforces the meal_<slug> id contract, derives
    // nutrition from the food library when the template has components, and
    // keeps hand-entered numbers when it does not.
    return window.LifeOSWrite.mealTemplate(t);
  }
  ...
  function upsertDietLog(patch) {
    // Canonical writer. Items are the truth: it derives the totals itself and
    // emits the two diet metrics from the same numbers, so this screen and
    // Today can no longer disagree about what a day is worth.
    return window.LifeOSWrite.dietLog(patch);
  }
  ...
  function wireTemplateForm() {
    ...
    form.addEventListener("submit", (e) => {
      ...
      // No id and no createdAt: the writer mints the one and stamps the
      // other, so this form cannot invent an id that breaks the naming
      // contract.
      const template = {
        name,
...
function initDayDraft(force = false) {
  ...
  function computeDraftTotals() {
    // The same code the writer totals a day with, so the preview on this
    // screen and the number that actually gets stored cannot drift apart.
    // Items already logged total from their frozen snapshots; items only just
    // added to the draft resolve live against the library, which is exactly
    // what a preview of an unsaved change should show.
    return window.LifeOSWrite.dietLogTotals(dayDraft?.items || []).totals;
  }
...
if (!dayDraft || dayDraft.date !== date) initDayDraft(true);
    ...
    // No totals and no metrics are computed here any more. The writer derives
    // both from the items, which is what stops this screen and Today from
    // holding different opinions about the same day.
    const stored = upsertDietLog({
      date,
      goal: dayDraft.goal,
      items: dayDraft.items.slice(),
    });

    // The draft has to pick up the snapshots the writer just froze. Without
    // this the next submit would hand the same items back unfrozen, and they
    // would be frozen again against whatever the library says by then, which
    // is precisely the retroactive rewrite the snapshots exist to prevent.
    dayDraft.items = (stored.items || []).slice();
