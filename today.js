...
(function () {
  ...
  function showInlineMetricForm(card, goal, metricDef) {
    ...
    form.addEventListener("submit", (e) => {
      ...
      // Save the metric entry through the canonical writer
      LifeOSWrite.metricEntry({
  ...
  function wireQuickMealLogging() {
    ...
    if (logBtn && selector) {
      logBtn.addEventListener("click", () => {
        ...
        // One call: append the item, derive the day's totals from all of the
        // items, freeze this meal at what it is worth right now, and emit both
        // diet metrics from those same totals.
        //
        // What this replaces is the M3 divergence itself. The old path read
        // the current metric and added this meal's macros to it, so Today
        // accumulated while the Log tab recomputed, and it stored a totals of
        // calories and protein only, silently dropping fat and carbs.
        const stored = LifeOSWrite.addDietLogItem(
          today,
          {
            slotId: slotId,
        ...
        document.dispatchEvent(new CustomEvent("lifeos:metrics-updated", {
          detail: { metricId: "diet_calories_kcal", date: today, value: stored.totals.calories }
        }));
        document.dispatchEvent(new CustomEvent("lifeos:metrics-updated", {
          detail: { metricId: "diet_protein_g", date: today, value: stored.totals.protein }
        }));
...
window.clockIn = function() {
  ...
  // Clocking in clears any previous end time for the day; minutes and note
  // are left as they are by the writer.
  LifeOSWrite.workLog({
    date: today,
    startTime,
    endTime: null,
