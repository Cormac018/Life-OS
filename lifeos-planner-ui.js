/* Reviewed day and week planning. Durable writes belong to PlannerOperations and the runtime. */
(function(global){
  'use strict';
  ...
  const routes=[['','No shortcut'],['train','Train'],['plan','Training programme'],['food','Food'],['sleep','Sleep'],['work','Work'],['goals','Goals'],['money','Money'],['people','People'],['life','Life planner'],['capture','Capture'],['preparation','Preparation']];
  ...
  const weekdayNames=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'],evidenceNames={'training-session':'workout session','sleep-record':'sleep record','meal-log':'logged meal','work-entry':'work shift','goal-progress':'goal progress','goal-action-event':'completed action','people-event':'people entry','money-transaction':'money record','preparation-completion':'preparation completion'};
  ...
  const starters=new Set(['profile','day-options','save-preview','add-slot','edit-slot','pin-slot','cancel-slot','status','done-slot','plan-week','plan-day','move-slot','alternative-slot','routine-new','routine-edit','routine-status','refresh-preparation']);
  ...
  // Scope is a UI hint kept with the unfinished proposal, never a new planner operation or an approval.
  function pendingPlan(){return planReview||(restored?(restored.kind==='proposal'?restored.intent:restored.intent.proposal):null);}
  function isDayPlan(plan=pendingPlan()){return plan?.options?.scope==='day';}
  function draftName(d){return d?.kind==='proposal'&&d.intent?.options?.scope==='day'?'proposed day':kindNames[d?.kind];}
  function dayClockError(plan=pendingPlan(),command=null,workspace=null,instant=nowIso()){
    if(!isDayPlan(plan)||(command&&!['plan','day'].includes(command.operation)))return null;
    const ws=workspace||context().workspace,p=plan.proposal,now=Date.parse(instant),today=planningDay({workspace:ws},instant);
    if(p.today!==today)return staleProposalMessage;
    const values=command?.operation==='plan'?command.entity.days.map(d=>d.value):command?.operation==='day'?[command.entity]:p.days.filter(d=>plan.selected instanceof Set?plan.selected.has(d.date):plan.selected.includes(d.date)).map(d=>d.value).filter(Boolean);
    for(const value of values){
      if(value.date!==today)continue;
      const proposed=p.days.find(d=>d.date===value.date),original=ws.domains.planner.days.find(d=>d.id===proposed?.previousVersionId)?.value;
      for(const slot of value.slots){
        if(slot.cancelled||slot.origin==='baseline')continue;
        const before=original?.slots.find(s=>s.id===slot.id&&!s.cancelled),changed=!before||before.start.at!==slot.start.at||before.end.at!==slot.end.at;
        if(changed&&Date.parse(slot.start.at)<now)return 'Time has moved on: '+slot.title+' would now start in the past. Propose again before saving. Your existing plan is unchanged.';
      }
    }
    return null;
  }
  ...
  function date(){if(!selectedDate){const ctx=context();selectedDate=planningDay(ctx)||ctx.today;}return selectedDate;}
  ...
  function weekAgenda(){
    const weekday=(new Date(date()+'T12:00:00Z').getUTCDay()+6)%7,monday=dateAdd(date(),-weekday),today=planningDay()||context().today;
  ...
  function kept(){return restored?draftName(restored):review?kindNames.command:editorDirty()?kindNames[editor.kind]:preview&&preview.date!==date()?'day preview':null;}
  ...
  function draftBanner(){
    if(!hasDraft())return '';
    const state=api.draftState?api.draftState():null,what=restored?draftName(restored):review?kindNames.command:editor?kindNames[editor.kind]:planReview?(isDayPlan()?'proposed day':kindNames.proposal):'day preview',onDevice=!!(restored||review||planReview||editorDirty()||preview);
  ...
  function weekStrip(){
    ...
    return '<div class="planner-week-control">'+button('previous-week','<span aria-hidden="true">‹</span><span class="sr">Previous week</span>','','icon-button')+'<span>'+esc(dateLabel(monday,{day:'numeric',month:'short'}))+' to '+esc(dateLabel(dateAdd(monday,6),{day:'numeric',month:'short'}))+'</span>'+button('next-week','<span aria-hidden="true">›</span><span class="sr">Next week</span>','','icon-button')+'</div><div class="planner-week">'+Array.from({length:7},(_,i)=>{
      ...
      return '<button type="button" data-planner-action="select-day" data-date="'+dayDate+'" aria-pressed="'+selected+'" aria-label="'+esc(dateLabel(dayDate)+(row?', saved plan':', no saved plan'))+'"><span>'+dateLabel(dayDate,{weekday:'short'})+'</span><strong>'+Number(dayDate.slice(8))+'</strong>'+(value?ribbon(value,'planner-week-ribbon'):'<div class="planner-week-ribbon"></div>')+'<small>'+(row?([...done.values()].filter(s=>s==='done').length?'✓ '+[...done.values()].filter(s=>s==='done').length:'Saved'):selected?'Preview':'Open')+'</small>'+(dayDate===today?'<i class="planner-today-dot" aria-hidden="true"></i>':'')+'</button>';
    }).join('')+'</div>';
  }
  ...
  function renderPage(){
    ...
    const ratio=Math.max(0,Math.min(1,a.occupiedMinutes/(a.dayMinutes||map.minutes))),circumference=2*Math.PI*43,planningToday=planningDay();
    return '<section id="plannerView"><header class="planner-header"><div><div class="eyebrow">Your time, with intention</div><h1>Day planner<span>.</span></h1><p>Give the important things a place. Keep room for real life.</p></div>'+'<div class="planner-header-actions">'+(p&&conductor()?button('plan-week','Plan my week','','button primary'):'')+button('profile',p?'Edit my setup':'Set up my week','','button ghost')+'</div></header>'+
      draftBanner()+
      weekStrip()+weekAgenda()+'<div class="planner-day-heading"><div><span class="planner-state '+(display.preview?'is-preview':'')+'">'+(display.preview?'Preview · not saved':'Saved plan')+'</span><h2>'+esc(dateLabel(day.date,{weekday:'long',day:'numeric',month:'long',year:'numeric'}))+'</h2></div><div class="planner-day-actions">'+(p&&conductor()&&planningToday&&day.date>=planningToday?button('plan-day',day.date===planningToday?'Replan from now':'Plan this day','','button primary'):'')+button('jump','Choose date','','text-button')+(day.date!==(planningToday||context().today)?button('today','Today','','text-button'):'')+'</div></div>'+
      ...
      '<p class="planner-footnote">Saved plans, routines, revisions and linked records stay on this device and are included in encrypted backups. Unfinished planner edits are kept on this device and in encrypted backups; they save nothing until you review and confirm them. Ask for a day or week proposal, then review the changes before saving. Plans do not change in the background, and closed-app reminders are still to come.</p></aside></div></section>';
  }
  ...
  function openPlan(options={},weekOf=null){
    const C=conductor();if(!C){api.toast('The planning service is not loaded.');return;}const ctx=context(),instant=nowIso(),target=weekOf||date(),{monday,sunday}=weekBounds(target),today=planningDay(ctx,instant),dayOnly=options.scope==='day';
    if(!today){api.toast('The current planning date could not be read. Review your time zone.');return;}
    if((dayOnly?target:sunday)<today){api.toast(dayOnly?'This day is history. Choose today or a coming day to plan.':'This week is history. Choose the current or a coming week to plan.');return;}
    const horizon=dayOnly?{from:target,to:target}:{from:monday<today?today:monday,to:sunday},{scope:scopeHint,...decisionOptions}=options;const r=C.propose({now:instant,horizon,workspace:ctx.workspace,revision:ctx.revision,options:decisionOptions});
    if(!r.ok){api.toast(r.error||(dayOnly?'The day could not be proposed.':'The week could not be proposed.'));return;}
    if(weekOf)selectedDate=weekOf;preview=null;restored=null;editor=null;review=null;planReview={proposal:r.proposal,options:copy(options),selected:new Set(r.proposal.days.filter(d=>!d.unchanged&&d.value).map(d=>d.date))};renderPlanReview();noteDraft();
  }
  function planDayHtml(day){
    ...
    return '<section class="planner-plan-day'+(included?' is-included':'')+'"><div class="planner-plan-day-head"><div><strong>'+esc(dateLabel(day.date,{weekday:'long',day:'numeric',month:'short'}))+'</strong><small>'+esc(nameOf(types,day.dayType))+' · '+esc(nameOf(scenarios,day.scenario))+(day.spareMinutes!==null?' · '+esc(duration(Math.max(0,day.spareMinutes)))+(isDayPlan()?' unallocated '+(day.date===planReview.proposal.today?'after the proposal time':'during waking hours'):' free'):'')+'</small></div>'+(changed?button('plan-toggle-day',included?'Included':'Excluded','data-date="'+day.date+'" aria-pressed="'+included+'"','button ghost planner-plan-toggle'):'<span class="planner-plan-nochange">No change</span>')+'</div>'+(day.value&&!isDayPlan()?ribbon(day.value):'')+
      (changes.length?'<ul class="planner-plan-changes">'+changes.map(c=>{const slot=day.value?.slots.find(s=>s.id===c.slotId),timing=c.from?'<small>'+esc(c.from)+' → '+esc(c.to)+'</small>':isDayPlan()&&slot&&['add','shorten'].includes(c.kind)?'<small>'+esc(slot.start.time)+' to '+esc(slot.end.time)+'</small>':'';return '<li><span class="planner-plan-kind is-'+esc(c.kind)+'">'+esc(changeNames[c.kind]||c.kind)+'</span><span><strong>'+esc(c.title)+'</strong>'+timing+'<small>'+esc(c.reason)+'</small></span></li>';}).join('')+'</ul>':(changed?'<p class="planner-help">Only anchors and unchanged activities.</p>':''))+
  ...
  function dayComparison(day,instant){
    const original=domain().days.find(row=>row.id===day.previousVersionId)?.value,before=original||{...emptyDay(day.date),slots:[]},after=day.value||before,bounds=ops().dayBounds(day.date,after.timeZone),now=Date.parse(instant),within=bounds.ok&&now>=bounds.start&&now<bounds.end;
    const rail=value=>'<div class="planner-day-comparison-rail">'+ribbon(value)+(within?'<span class="planner-now-line" style="left:'+((now-bounds.start)/(bounds.end-bounds.start)*100).toFixed(4)+'%" aria-hidden="true"></span>':'')+'</div>';
    return '<div class="planner-day-comparison"><div class="planner-comparison-title"><span>Your day, before and after</span><small>Full day · '+esc(after.timeZone)+'</small></div><div class="planner-comparison-row"><strong>Original <small>'+(original?'Last saved version':'No saved plan yet')+'</small></strong>'+rail(before)+'</div><div class="planner-comparison-row is-proposed"><strong>Proposed <small>Review before saving</small></strong>'+rail(after)+'</div>'+mapLabels(after)+'<div class="planner-comparison-key"><span><i style="background:'+colour.sleep+'"></i>Sleep</span><span><i style="background:'+colour.work+'"></i>Work</span><span><i style="background:'+colour.training+'"></i>Activities</span>'+(within?'<span><i class="planner-now-key"></i>Now</span>':'')+'</div></div>';
  }
  function renderDayPlanReview(error=''){
    const p=planReview.proposal,day=p.days[0],s=p.summary,instant=nowIso(),point=ops().endpointAt(Date.parse(instant),day?.value?.timeZone||profile()?.value.timeZone||zone()),proposedAt=ops().endpointAt(Date.parse(p.now),day?.value?.timeZone||profile()?.value.timeZone||zone()),sameDay=day.date===planningDay(context(),instant),keptRows=day.changes.filter(c=>c.kind==='keep');
    error=error||(proposalChanged(proposalToken(p))?staleProposalMessage:dayClockError());
    show(sameDay?'The rest of your day.':'A proposed day.','<div id="plannerPlanReview" class="planner-plan planner-plan-single"><span class="planner-state is-preview">One day · proposal · nothing saved</span><div class="planner-day-proposal-intro"><div><div class="eyebrow">'+esc(dateLabel(day.date,{weekday:'long',day:'numeric',month:'long'}))+'</div><h3>'+(sameDay?'Make room for what matters next.':'Give this day a workable shape.')+'</h3><p class="planner-intro">'+(sameDay?'Plan from now, with sleep, fixed commitments, completed activities and anything already started kept in place.':'Plan this date on its own, with sleep and fixed commitments protected.')+' Review every change before it becomes your plan. Actual records stay separate.</p></div><div class="planner-planning-clock"><span>Planning time now</span><strong>'+esc(point.ok?point.endpoint.time:'Unknown')+'</strong><small>'+esc(point.ok?dateLabel(point.endpoint.date,{day:'numeric',month:'short'}):'')+'</small></div></div>'+dayComparison(day,instant)+
      '<p class="planner-help">Proposed at '+esc(proposedAt.ok?proposedAt.endpoint.time:'the recorded planning time')+'. '+(sameDay?'If a changed activity would now start in the past, propose again.':'This proposal does not move work into another day.')+'</p>'+
      '<div class="planner-review-metrics"><strong>'+s.added+'<small>added</small></strong><strong>'+s.moved+'<small>moved</small></strong><strong>'+s.shortened+'<small>shortened</small></strong><strong>'+s.unscheduled+'<small>do not fit</small></strong></div>'+
      '<div class="planner-plan-controls">'+select('planScenario','Plan around',planReview.options.scenario||'',[['','Saved day hours (usual hours by default)'],['usual','Usual attendance'],['intended','Protected finish (intended hours)'],['future','Future commute']])+button('plan-again','Propose again','','button ghost')+'</div>'+
      ...
      '<div class="planner-day-protections"><strong>Kept in place</strong><p>Sleep, fixed anchors and recorded check-offs stay protected. Unfinished past activities remain visible for you to resolve.</p>'+(keptRows.length?'<ul>'+keptRows.map(c=>'<li><span>'+esc(c.title)+'</span><small>'+esc(c.reason)+'</small></li>').join('')+'</ul>':'')+'</div>'+
      '<h4 class="planner-history-title">What changes, and what needs your decision</h4>'+planDayHtml(day)+
      '<details class="planner-day-scenarios"><summary>Compare the shape of this day</summary><p class="planner-help">These totals cover the full day after sleep, work and commute anchors. They are not the time left from now and do not deduct your other activities.</p><div class="planner-scenarios">'+p.scenarios.map(x=>'<div class="planner-scenario'+(x.scenario===(planReview.options.scenario||day.scenario)?' is-active':'')+'"><strong>'+esc(duration(x.freeMinutes))+'<small>full-day anchor availability</small></strong><span>'+esc(x.label)+'</span><small>'+esc(duration(x.workMinutes))+' attendance · '+esc(duration(x.commuteMinutes))+' commuting</small>'+(x.remark?'<em>'+esc(x.remark)+'</em>':'')+(x.notes.length?'<em>'+esc(x.notes.join(' '))+'</em>':'')+'</div>').join('')+'</div></details>'+
      (p.queue.length?'<details class="planner-day-scenarios"><summary>Coming up, with lead times</summary><div class="planner-queue">'+p.queue.map(q=>'<div class="planner-queue-item"><span class="planner-plan-kind is-'+esc(q.status)+'">'+esc({due:'Due',overdue:'Overdue',placed:'In plan','beyond-horizon':'Later',unresolved:'Needs a decision'}[q.status]||q.status)+'</span><span><strong>'+esc(q.title)+'</strong><small>'+esc(q.eventTitle)+(q.kind==='preparation'?' · action due ':' on ')+esc(q.eventDate?dateLabel(q.eventDate,{day:'numeric',month:'short'}):'date to decide')+' · '+esc(q.reason)+'</small></span></div>').join('')+'</div></details>':'')+
      errorHtml(error)+'<div class="dialog-footer">'+button('pause','Close for now','','button ghost')+(planReview.selected.size?button('plan-apply','Review this day','','button primary'):'<span class="planner-help">'+(day.unchanged?'Your saved day already fits this proposal.':'Include the changed day to review it.')+'</span>')+'</div></div>');
  }
  function renderPlanReview(error=''){
    if(isDayPlan())return renderDayPlanReview(error);
  ...
  function applyPlan(){
    ...
    const clockError=dayClockError();if(clockError){renderPlanReview(clockError);return;}
  ...
  function openSlot(id){
    ...
    show(slot.title,'<div class="planner-slot-detail"><span class="planner-state">'+(display.preview?'Preview · not saved':'Saved plan')+'</span><h3>'+esc(timeLabel(slot.start))+' to '+esc(timeLabel(slot.end))+'</h3><p>'+esc(duration(elapsed))+' · '+esc(nameOf(categories,slot.category))+' · '+(slot.fixed?'Fixed timing':'Flexible timing')+(slot.window?' · usually '+esc(slot.window.start)+' to '+esc(slot.window.end):'')+(slot.cancelled?(to?' · moved to '+esc(dateLabel(to,{day:'numeric',month:'short'})):' · cancelled'):'')+'</p>'+occurrence+(event?'<p class="planner-reported'+(ops().checkoffState(slot,event)==='unverified'?' is-unverified':'')+'"><strong>'+esc(ops().checkoffState(slot,event)==='verified'?'Done · linked record':ops().checkoffState(slot,event)==='unverified'?'Checked off · no '+(sectionNames[slot.category]||'section')+' record linked':nameOf([['done','Checked off in planner'],['skipped','Skipped in planner'],['reset','Check-off reset']],event.status))+'</strong>'+esc(event.note||'No note recorded.')+(ops().checkoffState(slot,event)==='unverified'?' Link the real record from Done, or open '+esc(sectionNames[slot.category]||'the section')+' to log it.':'')+'</p>':'')+(slot.link?.preparation?'<p class="planner-help">This activity names one exact preparation action. Its facts stay in Preparation; a planner check-off alone never unlocks its next step.</p>'+button('start-slot','Open exact preparation action','data-id="'+esc(id)+'"')+button('refresh-preparation','Review current action and reopen','data-id="'+esc(id)+'"','text-button'):slot.link?button('navigate','Open '+esc(slot.link.label)+' <span aria-hidden="true">↗</span>','data-route="'+esc(slot.link.route)+'"','button ghost')+'<p class="planner-help">Section shortcut only. It does not identify a completed record.</p>':'')+planning+checkoff+'</div>');
  }
  function refreshPreparation(id){
    const display=displayed();if(display.preview||!display.saved){api.toast('Save or discard the preview before refreshing an action.');return;}
    const value=copy(display.value),slot=value.slots.find(s=>s.id===id);if(!slot?.link?.preparation)return;
    const ctx=context(),listed=global.PreparationOperations.list(ctx.workspace,ctx.today),row=listed.actions?.find(a=>a.purposeId===slot.link.preparation.purposeId);
    if(!row||row.status!=='ready'){api.toast(row?.reasons?.join(' ')||'This action is not ready. Review Preparation first.');return;}
    slot.link={route:'preparation',label:'Preparation',preparation:copy(row.binding)};slot.title=row.title;slot.category=row.category;slot.cancelled=false;
    prepareReview(makeCommand('day',value,display.previousVersionId),()=>openSlot(id),display.sourceDigest??null);
  }
  function showDone(id){
    ...
    const found=candidates(display.value.date).filter(c=>!slot.link?.preparation||c.kind==='preparation-completion'&&context().workspace.domains.preparation.events.some(e=>e.id===c.versionId&&e.binding.purposeId===slot.link.preparation.purposeId));
    const proposed=ops().proposeEvidence(slot,found,domain());doneProposals=proposed.ok?proposed.proposals:[];const spec=specialist.includes(slot.category);
  ...
  async function prepareReview(command,back,sourceDigest){
    ...
    try{
      ...
      const clockError=dayClockError(pendingPlan(),command);if(prepared.status!=='already-committed'&&clockError)throw new Error(clockError);
  ...
  function renderReview(error=review?.error||''){
    ...
    if(!error&&p.status!=='already-committed')error=dayClockError(pendingPlan(),r.command)||'';
  ...
  async function commit(){
    if(busy||!review)return;busy=true;setBusy(true);const chosen=review,reviewNode=$('plannerReview'),keepPlan=chosen.command.operation==='plan'?null:planReview,clockPlan=isDayPlan()?{proposal:copy(pendingPlan().proposal),options:copy(pendingPlan().options),selected:[...pendingPlan().selected]}:null;
    try{
      ...
      const clockError=dayClockError(clockPlan,chosen.command);if(chosen.prepared.status!=='already-committed'&&clockError)throw new Error(clockError);
      ...
      const beforeCommit=clockPlan?()=>{const error=dayClockError(clockPlan,command);if(error)throw new Error(error);}:undefined;
      const result=await api.commit(command,reviewDigest,{clearDraft:true,...(beforeCommit?{beforeCommit}:{})});
  ...
  function handleAction(target){
    ...
    if(action==='select-day')setDate(el.dataset.date);
    else if(action==='today')setDate(planningDay()||context().today);
    ...
    else if(action==='refresh-preparation')refreshPreparation(el.dataset.id);
    ...
    else if(action==='plan-day')openPlan({scope:'day'});
    else if(action==='plan-again'){const oldPlan=pendingPlan(),scenario=$('dialog')?.querySelector?.('[data-planner-field="planScenario"]')?.value??(oldPlan?.options.scenario||''),dayOnly=isDayPlan(oldPlan);
      ...
      const today=planningDay(context(),nowIso()),options={...(oldPlan?.options||{}),...(scenario?{scenario}:{})};if(!scenario)delete options.scenario;
      if(dayOnly){const target=oldPlan.proposal.horizon.from;openPlan(options,today&&target<today?today:target);}else openPlan(options,today&&weekBounds().sunday<today?today:null);}
  ...
  function submitForm(event){
    ...
    try{
      if(form.id==='plannerProfileForm'){syncProfile();prepareReview(makeCommand('profile',editor.data,editor.previousVersionId),()=>renderProfile());}
        ...
        const value=copy(editor.base),existing=value.slots.find(x=>x.id===editor.slotId),slot={id:editor.slotId,title:d.title,category:d.category,start:a.endpoint,end:b.endpoint,fixed:d.fixed,origin:editor.origin,cancelled:false,link:existing?.link?.preparation?copy(existing.link):d.route?{route:d.route,label:nameOf(routes,d.route)}:null,...(existing&&existing.anchor!==undefined?{anchor:copy(existing.anchor)}:{}),...(existing&&existing.window!==undefined?{window:d.releaseWindow?null:copy(existing.window)}:{})},i=value.slots.findIndex(x=>x.id===slot.id);
  ...
  global.PlannerUI=Object.freeze({configure,render,handleClick,openDate,planWeek:()=>handleClick({closest:()=>({dataset:{plannerAction:'plan-week'}})}),snapshotDraft,restoreDraft,refreshBanner,get busy(){return busy;}});
})(window);
