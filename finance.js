...
(function () {
  ...
  /* -------------------------
     One-time sign review

     Before the sign fix, the Quick Add buttons carried no direction, so an
     expense entered as a bare number was stored as positive and counted as
     income. Those records cannot be told apart from real income by any stored
     field, so nothing here guesses: every row is decided by the user.

     Deliberately scans the whole collection, ignoring the selected period. A
     period-scoped review would hide corrupted records outside the current
     window while looking complete, which is the failure this is meant to end.
     ------------------------- */

  function getDismissedSignIds() {
    const s = getSettings();
    const ids = s && Array.isArray(s.signReviewDismissedIds) ? s.signReviewDismissedIds : [];
    return new Set(ids);
  }

  function getSignReviewCandidates() {
    const dismissed = getDismissedSignIds();

    // isPayday and cat_salary cover every positive the app wrote on purpose,
    // so those are trusted income and never clutter the list.
    return getTransactions().filter((t) => {
      if (!t || !t.id) return false;
      if (!(Number(t.amount) > 0)) return false;
      if (t.isPayday === true) return false;
      if (t.categoryId === "cat_salary") return false;
      return !dismissed.has(t.id);
    });
  }

  function flipToExpense(t) {
    const before = Number(t.amount);
    const after = -Math.abs(before);

    // Confirm only on the flip. There is no edit UI for transactions, so a
    // mistake here costs a delete and re-add, losing createdAt. A dismiss is
    // cheap to reverse, so guarding both would dull the dialog that matters.
    const ok = confirm(
      `Change this to an expense?\n\n${t.date}  ${(t.merchant || "").trim() || "Transaction"}\n${money(before)}  becomes  ${money(after)}`
    );
    if (!ok) return;

    // The whole record goes back, not a patch: moneyTransaction replaces
    // rather than merges, so anything omitted here would be wiped.
    window.LifeOSWrite.moneyTransaction({ ...t, amount: after });

    refreshAll();
    Toast.success("Changed to an expense.");
  }

  function dismissFromSignReview(t) {
    const next = Array.from(getDismissedSignIds());
    if (!next.includes(t.id)) next.push(t.id);

    // No transaction is written. This records a decision to stop asking.
    upsertSettings({ signReviewDismissedIds: next });

    refreshAll();
    Toast.info("Left as is. It will not be flagged again.");
  }

  function renderSignReview() {
    const out = document.getElementById("moneySignReview");
    if (!out) return;

    const rows = getSignReviewCandidates()
      .slice()
      .sort((a, b) => String(b.date || "").localeCompare(String(a.date || "")));

    // Nothing left to check, so the card removes itself entirely.
    if (rows.length === 0) {
    ...
    const accById = new Map(getAccounts().map((a) => [a.id, a]));
    const catById = new Map(getCategories().map((c) => [c.id, c]));

    out.style.display = "block";
    out.innerHTML = `
      <div class="revolut-section-header">
        <div class="revolut-section-title">Check these amounts</div>
      </div>
      <div style="font-size:13px; color:var(--muted); margin-bottom:10px;">
        ${rows.length} transaction${rows.length !== 1 ? "s" : ""} recorded as money in. Older expenses could have been saved this way before the sign fix, so each one needs your decision.
      </div>
    `;
    ...
    rows.forEach((t) => {
      const acc = accById.get(t.accountId);
      const cat = catById.get(t.categoryId);
      const title = (t.merchant || "").trim() || cat?.name || "Transaction";
      const meta = [t.date, cat?.name, acc?.name].filter(Boolean).join(" • ");
      ...
      card.innerHTML = `
        <div class="revolut-card-header">
          <div style="flex:1;">
            <div class="revolut-card-title">${escapeHTML(title)}</div>
            <div class="revolut-card-subtitle">${escapeHTML(meta)}</div>
          </div>
          <div class="revolut-card-value positive">${escapeHTML(money(t.amount))}</div>
        </div>
        <div style="display:flex; gap:8px; margin-top:10px;">
          <button type="button" data-flip="${escapeHTML(t.id)}" style="flex:1; padding:8px 12px; background:none; border:1px solid var(--border); border-radius:8px; color:var(--danger); cursor:pointer; font-size:13px;">This was an expense</button>
          <button type="button" data-keep="${escapeHTML(t.id)}" style="flex:1; padding:8px 12px; background:none; border:1px solid var(--border); border-radius:8px; color:var(--muted); cursor:pointer; font-size:13px;">Leave as income</button>
        </div>
      `;

      card.querySelector("[data-flip]")?.addEventListener("click", (e) => {
        e.stopPropagation();
        flipToExpense(t);
      });
  ...
  function refreshAll() {
    ...
    // Not period-scoped on purpose, see renderSignReview.
    renderSignReview();
  ...
  function wireAddPaydayButton() {
    ...
    btn.addEventListener("click", () => {
      ...
      // The month check above is the guard for this path, so the writer's
      // own duplicate check stays off here.
      window.LifeOSWrite.moneyTransaction({
        date: paydayISO,
        accountId,
  ...
  // Shows which Quick Add button opened the form, so the direction is visible
  // before saving rather than only implied by the button already tapped.
  function setDirectionHint(direction) {
    const el = document.getElementById("moneyTxDirectionHint");
    if (!el) return;
    el.textContent = direction === "income" ? "Income" : "Expense";
  }
  ...
  function wireTxForm() {
    ...
    // Which button opened the form. The submit handler turns this into the
    // sign, so the user never types a minus. Defaults to expense: it is by far
    // the more common manual entry, and if a future path ever opens the form
    // without setting it, recording money out understates the balance, which
    // is the safer way to be wrong than inventing income.
    let pendingTxDirection = "expense";

    if (quickExpenseBtn && collapsible) {
      quickExpenseBtn.addEventListener("click", () => {
        pendingTxDirection = "expense";
        setDirectionHint(pendingTxDirection);
        collapsible.style.display = "block";
    ...
    form.addEventListener("submit", (e) => {
      ...
      // The button is the source of intent, so a typed minus cannot flip it.
      // Negative is money out, positive is money in, which is the convention
      // every reader of this collection already assumes.
      const magnitude = Math.abs(amount);
      const signedAmount = pendingTxDirection === "income" ? magnitude : -magnitude;

      // No duplicate check here on purpose: manual entry is never blocked.
      // The writer trims merchant and note and stamps the timestamps.
      window.LifeOSWrite.moneyTransaction({
        date,
        accountId,
        amount: signedAmount,
